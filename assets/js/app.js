...
// Initial Params
var paperData = null;  // Store the chart data
...
function scale(paperData, chosenAxis, xy) {
    var axisRange = (xy === "x") ? [0, chartWidth]:[chartHeight, 0]
    
    // create scales for chosen axis
    var linearScale = d3.scaleLinear()
      .domain([d3.min(paperData, d => d[chosenAxis]) * 0.8,
        d3.max(paperData, d => d[chosenAxis]) * 1.2
      ])
      .range(axisRange);
...
function updateChart() {
    ...
    if (value !== chosenAxis) {
        ...
        linearScale = scale(paperData, chosenAxis, xy);
...
function init() {
    ...
    var xLinearScale = scale(paperData, chosenXAxis, "x");
    var yLinearScale = scale(paperData, chosenYAxis, "y");
    ...
    var yLabels = chartGroup.append("g")
        .attr("transform", `rotate(-90 ${(margin.left / 2)} ${(chartHeight/2) + 40})`)
        .classed("atext", true)
        .attr("id", "yLabels");
